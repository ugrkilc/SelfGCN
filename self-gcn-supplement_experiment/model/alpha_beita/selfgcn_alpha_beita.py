...
class SelfGCN_Block(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        df2 = pd.DataFrame(att)

        writer = pd.ExcelWriter('matrices.xlsx')

        # 将 DataFrame 写入 Excel 文件
        df1.to_excel(writer, sheet_name='x_1', index=False)
        df2.to_excel(writer, sheet_name='att', index=False)
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, adaptive=True, residual=True):
        ...
        # self.alpha = torch.tensor(0.0000015, requires_grad=False)
