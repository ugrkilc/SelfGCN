...
class SelfGCN_Block(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        x_1 = A.unsqueeze(0).unsqueeze(0) if A is not None else 0 # N,out_channels,V,V


        q , k = x1, x2 # rel_channels
        att = self.tanh(torch.einsum('nctu,nctv->ncuv',[q,k])/T) # rel_channels
        att = self.att_t(att) # out_channels
        x_1 = x_1.repeat(att.shape[0],att.shape[1],1,1)
        zero_vec = -9e15 * torch.ones_like(att)
        att_res = torch.where(x_1>0,att,zero_vec)
        x1_res = torch.einsum('ncuv,nctv->nctu', att_res, x3) # out_channels
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, adaptive=True, residual=True):
        ...
        self.num_subset = numpy_array.shape[0]
        ...
        if self.adaptive:
            self.PA = nn.Parameter(torch.from_numpy(numpy_array.astype(np.float32)))
        else:
            self.A = Variable(torch.from_numpy(numpy_array.astype(np.float32)), requires_grad=False)
