...
class SelfGCN_Block(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        att = torch.einsum('nctu,nctv->ncuv', [q, k]) / T  # rel_channels
        att = self.att_t(att)  # out_channels
        # zero_vec = -9e15 * torch.ones_like(att)
        att_res = torch.where(x1_res >= 0, att, x1_res)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, kernel_size=5,
        ...
        if not residual:
            ...
            self.residual = unit_tcn(in_channels, out_channels, kernel_size=1,
                                     stride=stride)  # 如果输入的通道跟输出的通道不一致的话，那么残差的通道要与输出的通道一致，则使用1x1的卷积
